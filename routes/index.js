...
router.get('/', function(req, res) {
    ...
    student_record.findOne({
        id: req.session.passport.user
    }).exec(function(err, student) {
        //find courses completed
        courses_completed.find({
            ...
            if (req.session.passport.user == "12345678") {
                ...
                courseprereq.find({}).sort({
                    order: 1
                }).exec(function(err, coursep) {
                    res.render('index', {
                        user: req.user,
                        name: student,
                        student_info: user,
...
router.post('/courses_completed', function(req, res) {
    ...
    course.save(function(err) {
        ...
        res.json({
            message: 'course completed added for the student'
        });
    });
});
...
router.post('/register', function(req, res, next) {
    Login_info.register(new Login_info({
